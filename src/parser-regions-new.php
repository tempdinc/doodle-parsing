...
file_put_contents(LOG_DIR . '/parser-regions-new.log', '[' . date('Y-m-d H:i:s') . '] Parse regions start >>> ' . PHP_EOL, FILE_APPEND);
...
foreach ($citiesDB as $states) {
    foreach ($states as $code => $citiesArray) {
        foreach ($citiesArray as $city) {
            ...
            file_put_contents(LOG_DIR . '/parser-regions-new.log', ' | ' . $region . ' > ' . $region_slug, FILE_APPEND);
...
file_put_contents(LOG_DIR . '/parser-regions-new.log', PHP_EOL, FILE_APPEND);
...
file_put_contents(LOG_DIR . '/parser-regions-new.log', ' total added cities - ' . $counter . PHP_EOL . ' END >>> ' . date('Y-m-d H:i:s'), FILE_APPEND);
...
function insertNewTerm($region_name, $region_slug)
{
    ...
    if (is_wp_error($insert_res)) {
        file_put_contents(LOG_DIR . '/parser-regions-new.log', ' | ERROR ADDING NEW REGION: ' . $insert_res->get_error_message(), FILE_APPEND);
        echo $insert_res->get_error_message();
        return false;
    } else {
        file_put_contents(LOG_DIR . '/parser-regions-new.log', ' | NEW REGION: ' . $region_name . ' > ' . $region_slug, FILE_APPEND);
