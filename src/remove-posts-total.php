...
echo 'Total properties WHERE post_id IS NOT NULL - ' . $total_properties . PHP_EOL;
...
echo 'Properties existing posts unique - ' . count($property_existing_posts) . PHP_EOL;
...
echo 'Total availabilities WHERE post_id IS NOT NULL - ' . $total_availabilities . PHP_EOL;
...
echo 'Availability existing posts unique - ' . count($availability_existing_posts) . PHP_EOL;
...
sort($existing_posts);
echo 'Total (availability & property) existing posts unique - ' . count($existing_posts) . PHP_EOL;
...
echo 'Total post ids with listing type ' . $listing_type . ': ' . $total_posts . PHP_EOL;
file_put_contents(LOG_DIR . '/remove-posts-total.log', PHP_EOL . 'Total post ids with listing type ' . $listing_type . ': ' . $total_posts . PHP_EOL, FILE_APPEND);
$limit = 100;
...
file_put_contents(LOG_DIR . '/remove-posts-total.log', PHP_EOL . 'Posts to remove from wp_posts table - ' . count($posts_to_remove) . PHP_EOL, FILE_APPEND);


// 4. Removing availability records with NON existing post_ids
file_put_contents(LOG_DIR . '/remove-posts-total.log', '[' . date('Y-m-d H:i:s') . '] 4. Removing availability records with NON existing post_ids' . PHP_EOL, FILE_APPEND);
...
file_put_contents(LOG_DIR . '/remove-posts-total.log', PHP_EOL . 'Posts to remove from availability table - ' . count($availability_existing_posts_to_remove) . PHP_EOL, FILE_APPEND);
...
// 5. Removing properties records with NON existing post_ids
file_put_contents(LOG_DIR . '/remove-posts-total.log', '[' . date('Y-m-d H:i:s') . '] 5. Removing properties records with NON existing post_ids' . PHP_EOL, FILE_APPEND);
$property_existing_posts_to_remove = array_unique($property_existing_posts_to_remove);
sort($property_existing_posts_to_remove);
file_put_contents(LOG_DIR . '/remove-posts-total.log', PHP_EOL . 'Posts to remove from properties table - ' . count($property_existing_posts_to_remove) . PHP_EOL, FILE_APPEND);
...
// 6. Fixing & clearing availability records: bedroom_cnt, bathroom_cnt, listing_price, home_size_sq_ft
file_put_contents(LOG_DIR . '/remove-posts-total.log', '[' . date('Y-m-d H:i:s') . '] 6. Fixing & clearing availability records: bedroom_cnt, bathroom_cnt, listing_price, home_size_sq_ft' . PHP_EOL, FILE_APPEND);
...
// 7. Removing availability records with post_id IS NULL && NOT Available statuses
file_put_contents(LOG_DIR . '/remove-posts-total.log', '[' . date('Y-m-d H:i:s') . '] 7. Removing availability records with post_id IS NULL && NOT Available statuses' . PHP_EOL, FILE_APPEND);
$query = $parsing_db->pdo->prepare("SELECT count(*) FROM `availability` WHERE 1=1 LIMIT 1");
$query->execute();
$total_availabilities = $query->fetchColumn();
echo 'Total availabilities - ' . $total_availabilities . PHP_EOL;
...
// 8. Removing availability records with post_id IS NULL && WRONG listing_price & WRONG home_size_sq_ft
file_put_contents(LOG_DIR . '/remove-posts-total.log', '[' . date('Y-m-d H:i:s') . '] 8. Removing availability records with post_id IS NULL && WRONG listing_price & WRONG home_size_sq_ft' . PHP_EOL, FILE_APPEND);
$query = $parsing_db->pdo->prepare("SELECT count(*) FROM `availability` WHERE 1=1 LIMIT 1");
$query->execute();
$total_availabilities = $query->fetchColumn();
echo 'Total availabilities - ' . $total_availabilities . PHP_EOL;
...
// 9. Count availability records with WRONG listing_price & WRONG home_size_sq_ft
file_put_contents(LOG_DIR . '/remove-posts-total.log', '[' . date('Y-m-d H:i:s') . '] 9. Count availability records with WRONG listing_price & WRONG home_size_sq_ft' . PHP_EOL, FILE_APPEND);
$sql_query = "SELECT count(*) FROM `availability` WHERE `listing_price` IS NULL OR `listing_price` LIKE '%person%' OR `listing_price` LIKE '%call%' OR `home_size_sq_ft` = '' OR `home_size_sq_ft` IS NULL OR `image_urls` = '' OR `image_urls` IS NULL LIMIT 1";
$query = $parsing_db->pdo->prepare($sql_query);
$query->execute();
$total_availabilities = $query->fetchColumn();
echo 'Total availabilities - ' . $total_availabilities . PHP_EOL;
file_put_contents(LOG_DIR . '/remove-posts-total.log', PHP_EOL . "Total availability records with WRONG listing_price & WRONG home_size_sq_ft & image_urls  = ' - " . $total_availabilities . PHP_EOL, FILE_APPEND);


// 10. Getting property_ids && post_ids FROM availability table with post_id = 0 & WRONG listing_price & WRONG home_size_sq_ft & image_urls  = ''
file_put_contents(LOG_DIR . '/remove-posts-total.log', "[" . date('Y-m-d H:i:s') . "] 10. Getting property_ids && post_ids FROM availability table with post_id = 0 & WRONG listing_price & WRONG home_size_sq_ft & image_urls  = ''" . PHP_EOL, FILE_APPEND);
$properties_ro_remove = [];
$pages = intdiv($total_availabilities, 100);
for ($i = 0; $i <= $pages; $i++) {
    $start = $i * 100;
    $query = $parsing_db->pdo->prepare("SELECT property_id, post_id FROM `availability` WHERE `post_id` = 0 OR `post_id` = '0' OR `listing_price` IS NULL OR `listing_price` LIKE '%person%' OR `listing_price` LIKE '%call%' OR `home_size_sq_ft` = '' OR `home_size_sq_ft` IS NULL OR `image_urls` = '' OR `image_urls` IS NULL LIMIT $start,100");
    $query->execute();
